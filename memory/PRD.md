...
- ✅ **Sports Outfit Configurator** (Feb 2026) — public `/sports-outfit-configurator` for gyms/PTs/boxing/thai/kick/dance. Pick Training kit and/or Tracksuit; print modes mutually exclusive (Unbranded / Breast logo / Back print / Full front). NEW: **"Different sizes for tops & bottoms?"** toggle per section for shape mismatches — max(top qty, bottom qty) counts as kit qty. Backing endpoint: GET /api/sports-outfit/config.
